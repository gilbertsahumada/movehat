...
- **TypeScript integration tests** (`movehat test --ts`) — the suite boots a real local node (one shared movelite per run when available, or a full Movement node) plus compiles and deploys your modules. Useful but slower than Move unit tests.
...
# at the bottom) because they boot a local node and compile + deploy
# the Move modules, adding minutes of latency on a full Movement node
# (movelite, where available, cuts the node boot to under a second).
...
jobs:
  test:
    ...
    steps:
      ...
      # Opt-in: TypeScript integration tests. The suite boots one local
      # node (shared across specs with movelite; ~15-30s per spec on a
      # full Movement node) and compiles + deploys your modules, so it
      # can take minutes. Uncomment when your project has
...
- The CI minutes are acceptable (the suite boots one shared node — ~15-30s once on a full Movement node — plus Move compilation and deploys per spec).
