...
The generated `scripts/deploy-counter.ts` is local-first — with no network
selected it deploys to a disposable local chain. For this tutorial, adapt it
into a live-only variant that refuses to run without an explicit public
network:
