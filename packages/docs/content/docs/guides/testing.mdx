...
### One shared node per process (automatic)

When movelite is the selected backend, node sharing needs zero
configuration: the first fixture in a `mocha` run boots one movelite node,
every later fixture in the same process reuses it, and the node shuts down
automatically when the process exits. Your test files contain no
node-lifecycle code at all — no setup file, no `localNode` wiring, no
explicit stop. Each fixture still gets its own accounts and deployments,
so specs stay isolated.

To opt out of the shared node:

- Pass your own `localNode` — movehat uses the node you hand it and never
  stops it.
- Pass any `node*` option (`nodeApiPort`, `nodeTestDir`, ...) — these
  configure the full Movement node, and movelite does not consume them,
  but passing one always marks the fixture as wanting a private node: it
  gets its own instance, owns it, and stops it on `teardown()`.
- Disable movelite (`useMovelite: false` or `MOVEHAT_USE_MOVELITE=0`) — the
  full Movement node always runs per-fixture.
...
> construct a `MoveliteManager`; a plain `LocalNodeManager` gives only the
> degraded event/state trace.

> **mocha `--parallel` is not supported.** Node sharing is per-process, so
> each parallel worker boots its own movelite node, and only two ports
> (8090, then 8091) are tried before startup fails with
> `Ports 8090 and 8091 are in use`. Run your suite in a single mocha
> process (the default) — the shared node already keeps sequential specs
> fast.
