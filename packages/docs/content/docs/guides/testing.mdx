...
- Pass your own `localNode` — movehat uses the node you hand it and never
  stops it.
- Pass any `node*` option (`nodeApiPort`, `nodeTestDir`, ...) — a node with
  a specific configuration can't be shared, so that fixture gets a private
  one it owns and stops on `teardown()`.
