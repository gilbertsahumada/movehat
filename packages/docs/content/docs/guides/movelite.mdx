...
- **Your tests inject a node** — passing `localNode` to `Harness.createLocal`
  ...
  whatever node you hand it. Older scaffolds shipped a `tests/setup.ts` root
  hook that pinned a node this way — current movehat shares one movelite
  node per process implicitly, so that file can simply be deleted (see
  [One shared node per
  process](./testing#one-shared-node-per-process-automatic)).
...
Movelite discovery checks `MOVELITE_PATH` first (relative paths resolve from the
project), then Movehat's optional platform package, then `PATH`. Local
`node_modules/.bin` and global npm installations are supported. An invalid
explicit `MOVELITE_PATH` is a configuration error; unavailable automatic
candidates fall back to Movement node.
