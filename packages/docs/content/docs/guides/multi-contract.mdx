...
describe("Registry Contract", () => {
  ...
  it("is_registered flips false → true after register", async () => {
    const carol = harness.accounts.alice!;
    ...
    // A brand new unregistered address — created on the harness's
    // own AccountManager (so it's scoped to this Harness instance).
    const newAccount = harness.runtime.accountManager.createAccount();
...
- **`accountLabels` must include `"deployer"`** — `Harness.createLocal` uses that label to bootstrap the runtime. Other labels (`alice`, `bob`) are arbitrary identifiers your test body reads via `harness.accounts.<label>`.
...
- **`harness.cleanup()` in `after`** releases the harness (further calls throw). It does not stop the shared movelite node — the next spec reuses it, and isolation comes from each fixture's own accounts and deployments, not from a fresh chain.
...
The first spec boots one shared local node; each spec then publishes its relevant module with its own accounts, runs its tests, and releases its harness — the node itself stops at process exit. You'll see the spinner-driven setup output described in the [Console output](/docs/guides/console-output) guide.
...
- Each test spec is independent: its own labeled accounts and its own deployment cache entry — while sharing one local node across the process.
