...
mainnet does not require `PRIVATE_KEY`. An RPC API key only authenticates
reads and is never a signing account — and it must be passed explicitly, e.g.
`Harness.createFork("mainnet", process.env.MOVEMENT_API_KEY)`; Movehat does
not read `MOVEMENT_API_KEY` from the environment on its own.
