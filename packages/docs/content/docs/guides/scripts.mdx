...
> the script executes through the TypeScript SDK instead of the
> Movement CLI, because the CLI cannot parse movelite's responses. Two
> constraints follow: the script must be a pre-compiled `.mv`
> (`movement move compile` emits them under
> `build/<pkg>/bytecode_scripts/`), and `args` supports scalar types
> only (`bool`, `u8`–`u256`, `address`, `string`, `hex`). Both are
> lifted when running against a full Movement node
