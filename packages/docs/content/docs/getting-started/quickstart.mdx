...
describe("Counter Contract", () => {
  ...
  after(async () => {
    // Poisons the harness — any further method call (other than
    // cleanup) throws HarnessDisposedError. The shared local node
    // keeps running for later specs and stops at process exit.
