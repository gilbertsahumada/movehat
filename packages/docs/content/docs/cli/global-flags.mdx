...
The effective precedence is `--network` / `MH_CLI_NETWORK`, then
`MOVEHAT_NETWORK`, then `MH_DEFAULT_NETWORK`, then `defaultNetwork` in
`movehat.config.ts`.
...
| `MH_DEFAULT_NETWORK=<name>` | — | Lower-priority environment default used before project config |
