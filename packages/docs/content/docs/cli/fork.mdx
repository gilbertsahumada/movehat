...
The server stays pinned to the snapshot it loaded. If another process
overwrites or resets the same fork, subsequent requests return HTTP `409`
with `error_code: "fork_snapshot_changed"`. Restart `movehat fork serve` to
adopt the replacement snapshot; it is never reloaded silently.
